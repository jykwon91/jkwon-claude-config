...
- When a config change isn't taking effect on a server, check docker BEFORE editing host-side files — host configs (`/etc/caddy/Caddyfile`, `/etc/postgresql/*`, systemd units) are routinely eclipsed by container-mounted equivalents and can sit unused for months. Run `docker ps`, `ss -tlnp | grep <port>`, and `docker inspect --format '{{ range .Mounts }}{{ .Source }} -> {{ .Destination }}{{ println }}{{ end }}'` to identify which file the running process actually reads, then edit that file (and `docker compose restart <service>`).
- Never trust auto-memory as ground truth for load-bearing decisions at session start — before dispatching code-shipping work, validate the relevant claims (which repo deploys to prod, which features exist, which branches are active, what's currently broken) against the live codebase and ground-truth signals (`docker ps`, `docker compose ls -a`, `git log`, `gh pr list`, deployed `/api/openapi.json`). Memory is point-in-time observation; the cost of one verification check is seconds, the cost of acting on stale memory is rework, broken deploys, or destroyed data.
- When a fix you shipped doesn't reach the user — they still see the old behavior after deploy goes green — STOP and verify the fix actually reached production before shipping any follow-up. Verify in this order: (1) live HTTP response from the public URL contains your change (`curl -sSI`/`curl -sS` from outside the deploy host, parse the actual bytes/headers); (2) the container or host filesystem at the serving path matches the new code (`docker compose exec ... cat`/`ls`); (3) only then re-read your source as a possible cause. Cascading wrong fixes — each shipped after the previous one didn't work, without verification — is the highest-cost failure mode: three architecturally-broken fixes in production beats one careful diagnosis on every dimension. Recognition signal: "I shipped X, deploy ran green, user still sees old behavior" → suspect the deploy pipeline (volume staleness, build cache, browser SW, file_server pointing at a stale dir) before re-debugging the code.
