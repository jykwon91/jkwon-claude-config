...
- Always write and run E2E tests for every new feature before committing — verify E2E test files are staged alongside feature code, run the full E2E suite, and confirm a green result before proceeding. Unit tests alone are never sufficient validation for user-facing changes. Tests must cover the full user flow (form submission, API interaction, state changes, error handling), not just rendering or visibility checks.
...
- When multiple features need to be developed in parallel (e.g., multiple Claude sessions on the same repo), use git worktrees to isolate each session's work. Never have two sessions modifying files in the same working directory — use `git worktree add` to create an isolated copy. Only one session should create database migrations at a time.
