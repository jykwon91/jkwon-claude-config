...
copilot/
  ...
  agents/                    # 22 custom agents (-> ~/.copilot/agents/)
  ...
  skills/                    # 4 skills (-> ~/.copilot/skills/)
...
- **Preference management skills** (add/update/delete-preference) -- these manage a shared config repo
...
- **Model pinning** -- `model: opus` / `model: sonnet` in agent frontmatter (not yet supported in Copilot CLI)
  - In Claude Code: g-build-feature, g-pre-commit, g-audit-security, g-debug-bug, g-design-architecture, g-design-security, g-qa, g-tech-debt-scan use opus; all others use sonnet
