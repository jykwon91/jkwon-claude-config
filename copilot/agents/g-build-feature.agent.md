...
- **Build a project context string** to pass to all sub-agents (avoids each agent re-detecting the stack):
  ...
  - Stack guides: e.g., "react, fastapi, python"
...
5a. **Code Review:** Review all files changed during this pipeline (`git diff --name-only HEAD`). Must Fix: logic errors, missing await, data loss patterns, security issues, schema mismatches. Note but don't fix: performance concerns, large components, missing annotations.

5b. **Final Validation:** Re-run all test suites. Collect results.

After both complete:
- If code review found Must Fix issues AND tests passed: fix the issues, re-run affected suites
- If no Must Fix AND tests passed: advance to Stage 7
- If tests failed: go back to the relevant stage

**Stage 7 — Fix existing tech debt (conditional):**
- Read `## Tech Debt Policy` in project instructions
