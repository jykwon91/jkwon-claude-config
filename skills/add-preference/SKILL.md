...
7. In `~/Documents/Git/jkwon-claude-config` (if pushing to global):
   ...
   - **If pushing to an existing PR, update its title and body** to reflect all changes on the branch (see PR auto-update below)
8. Return the PR URL (or confirm local addition) to the user.

## PR auto-update

When pushing to an existing PR, the title and body become stale (they only describe the first change). After every push to an existing PR:
...
3. **Update the PR body** with a bullet list generated from the commit messages:
   ...
   - ...
   ```
4. Use `gh pr edit <number> --title "<title>" --body "<body>"` to update
